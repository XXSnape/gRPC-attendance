...
import QrCodeDisplay from "./QrCodeDisplay"; // Импорт из отдельного файла
...
export default function LessonDetail() {
    ...
    try {
      ...
      response.data.groups.forEach((group) => {
        if (group.attendances && group.attendances.length > 0) {
          ...
          originalStates[group.id] = JSON.parse(
            JSON.stringify(group.attendances)
          );

          const present = group.attendances.filter(
            (a) => a.attendance.status === 0
          ).length;
  ...
  const handleMarkAll = (groupId) => {
    ...
    const allMarked = groupAttendance.every(
      (student) => student.attendance.status === 0
    );
  ...
  const handleCancel = (groupId) => {
    const updatedAttendances = { ...groupAttendances };
    updatedAttendances[groupId] = JSON.parse(
      JSON.stringify(originalAttendances[groupId])
    );
    setGroupAttendances(updatedAttendances);
  };
  ...
  const handleSaveAttendance = async (groupId) => {
    ...
    try {
      ...
      const studentMap = new Map(
        updatedAttendances.map((item) => [item.student_id, item.attendance])
      );
      ...
      const present = newAttendances.filter(
        (a) => a.attendance.status === 0
      ).length;
      ...
      if (currentStudent) {
        const updatedStudentAttendance = studentMap.get(
          currentStudent.student_id
        );
        if (updatedStudentAttendance) {
          const updatedLessonData = { ...lessonData };
          updatedLessonData.schedule_data.student_data.attendance =
  ...
  const handleGrantAccess = async (minutes = null, groupId = null) => {
    try {
      ...
      messageApi.success(
        minutes === null ? "Доступ отозван" : `Доступ выдан на ${minutes} минут`
      );
  ...
  const getAttendanceColumns = (groupId) => [
      ...
      render: (text, record) => (
        <div className="flex items-center justify-center">
          ...
          <Tooltip title={`Персональный номер: ${record.personal_number}`}>
            <span
              className={record.is_prefect ? "underline font-semibold" : ""}
            >
              {text}
            </span>
          </Tooltip>
    ...
    },
    {
      ...
      render: (attendance, record, index) => {
        ...
        const canEdit =
          isTeacher ||
          (group?.can_be_edited_by_prefect && isCurrentStudentPrefect);

        return (
          <Radio.Group
            value={attendance.status}
            onChange={(e) =>
              handleAttendanceChange(groupId, index, e.target.value)
            }
  ...
  const collapseItems = displayGroups.map((group) => {
    const attendance = groupAttendanceStats[group.id] || {
      present: 0,
      total: 0,
    };
            ...
            <Descriptions column={2} size="middle">
              ...
              {currentStudent && (
                <Descriptions.Item label="Статус">
                  <Tag
                    color={getStatusColor(currentStudent.attendance.decryption)}
                  >
